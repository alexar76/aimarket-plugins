...
Receipts conform to [**AWR/2**](../../../awr/SPEC.md) (Agent Work Receipt 2.0.0). Verification needs no network, no registry, no blockchain and no issuer-specific software: any conformant W3C VC library can check one, and so can a 100-line implementation.
...
| [Receipt format](docs/receipt-format.md) | The AWR/2 document this plugin emits, field by field |
...
| [AWR/2 specification](../../../awr/SPEC.md) | The normative format definition |
...
Field-by-field, including the hub-specific members (`providerHub`, `payloadSerialization`, `reputationScore`, `hubInfo`) and how each AWR/1 field maps: [docs/receipt-format.md](docs/receipt-format.md).
