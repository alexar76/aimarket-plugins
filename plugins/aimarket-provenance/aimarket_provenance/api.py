...
def create_provenance_router(
    ...
    async def verify(receipt_id: str) -> dict[str, Any]:
        ...
        # Bind this hub's own identity to its signing key: a stored receipt that
        # claims to be issued by us must carry our real key, not a self-provided one.
        result = verify_receipt(
            receipt, signer,
            trusted_issuer_keys={hub_name: signer.public_key_b64},
        )
        return {
