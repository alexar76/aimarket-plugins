...
def verify_receipt(
    ...
    2b. Issuer binding — if ``trusted_issuer_keys`` (issuer / provider_hub → pinned
        public key) is supplied, the embedded key MUST match the pinned key for the
        claimed issuer; a forged receipt carrying its own keypair is rejected.
        Without a registry the embedded key is only self-asserted (not issuer-bound).
    ...
    # 2b. Issuer binding — the signature above only proves that *some* keypair signed
    # the receipt, not that the keypair belongs to the claimed provider_hub. When a
    # trusted issuer→key registry is supplied, require the embedded key to match the
    # pinned key for the claimed issuer (rejects self-signed forgeries). Without a
    # registry the key is self-asserted and cannot be bound to an identity.
    issuer = receipt.provider_hub or ""
    pinned = (trusted_issuer_keys or {}).get(issuer)
    if pinned is not None:
        bound = pinned == receipt.issuer_public_key_b64
        result.checks.append({
            "check": "issuer_binding", "passed": bound, "issuer": issuer, "bound": bound,
        })
        if not bound:
            result.errors.append(
                f"Issuer key mismatch: embedded key is not the pinned key for {issuer}"
            )
    else:
        result.checks.append({
            "check": "issuer_binding", "passed": True, "issuer": issuer, "bound": False,
            "note": "self-asserted key — no trusted registry for this issuer",
        })
