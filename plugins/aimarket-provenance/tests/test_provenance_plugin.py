...
def client(storage, signer) -> TestClient:
    app = FastAPI()
    # Match Hub's deny-by-default global CORS layer. The receipt route's narrow
    # response header must survive it without opening any state-changing route.
    app.add_middleware(
        CORSMiddleware,
        allow_origins=[],
...
def _route_paths(router, _seen=None):
    """Every endpoint path under *router*, however FastAPI happens to nest it.

    FastAPI 0.128 put the endpoints straight in `router.routes`; 0.141 wraps whatever
    `include_router()` added in an `_IncludedRouter`, which carries no `.path` and exposes
    the real router as `original_router`. Reading `.path` off every entry passed on the
    developer's pinned version and raised AttributeError in the shipping image — a route
    test that only holds on one FastAPI version says nothing about what gets deployed, and
    this plugin is deployed in an image two minor versions ahead of the dev venv.
    """
    ...
    for route in getattr(router, "routes", ()):
        ...
        for attribute in ("original_router", "router", "app"):
            ...
            if nested is not None and hasattr(nested, "routes"):
                paths |= _route_paths(nested, seen)
                break
    return paths
...
class TestPluginHooks:
    ...
    def test_auto_receipt_generation(self, plugin, db, monkeypatch, tmp_path):
        ...
        pr = plugin.on_invoke_receipt(output, context)
        ...
        assert stored.provider_hub == "local"
        assert stored.latency_ms == 1500
        assert stored.price_usd == 0.05
        assert stored.invocation_nonce == "rcpt_demo_1"
        assert stored.settlement["holdId"] == "rcpt_demo_1"
        assert stored.settlement["amount"] == {"currency": "USD", "amount": "0.05"}
    ...
    def test_verify_url_is_relative_without_a_hub_url(self, plugin, db, monkeypatch, tmp_path):
        ...
        plugin.register_routes(APIRouter())
        output: dict = {}
        pr = plugin.on_invoke_receipt(output, {"capability_id": "c", "input": {}})
        assert pr["verify_url"].startswith(
...
class TestApi:
    ...
    def test_receipt_and_verify_round_trip(self, client):
        ...
        assert fetched.headers["content-type"].startswith("application/vc")
        assert fetched.headers["cache-control"] == "public, max-age=31536000, immutable"

        browser_fetch = client.get(
            "/ai-market/v2/p/provenance/receipt/%s" % (receipt_id,),
            headers={"Origin": "https://verify.modelmarket.dev"},
        )
        assert browser_fetch.headers["access-control-allow-origin"] == (
            "https://verify.modelmarket.dev"
        )
        portal_fetch = client.get(
            "/ai-market/v2/p/provenance/receipt/%s" % (receipt_id,),
            headers={"Origin": "https://use.modelmarket.dev"},
        )
        assert portal_fetch.headers["access-control-allow-origin"] == (
            "https://use.modelmarket.dev"
        )
        untrusted_fetch = client.get(
            "/ai-market/v2/p/provenance/receipt/%s" % (receipt_id,),
            headers={"Origin": "https://attacker.example"},
        )
        assert "access-control-allow-origin" not in untrusted_fetch.headers
