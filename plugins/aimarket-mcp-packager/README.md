...
<!-- aicom-readme-badges -->
<p align="center">
  <a href="https://github.com/alexar76/aimarket-plugins/actions/workflows/ci.yml"><img src="docs/badges/ci.svg" alt="CI" /></a>
...
Quality score: **[glama.ai/mcp/servers/alexar76/aimarket-plugins/score](https://glama.ai/mcp/servers/alexar76/aimarket-plugins/score)** ·
[![alexar76/aimarket-plugins MCP server](https://glama.ai/mcp/servers/alexar76/aimarket-plugins/badges/score.svg)](https://glama.ai/mcp/servers/alexar76/aimarket-plugins)
...
Transport: **stdio** (`aimarket_mcp_packager/stdio_server.py`). Built with the official **[MCP Python SDK](https://github.com/modelcontextprotocol/python-sdk)** (`mcp` package, `FastMCP`).
...
| Item | Location |
|------|----------|
| MCP entrypoint | `aimarket_mcp_packager/stdio_server.py` |
| MCP SDK dependency | [`requirements-mcp.txt`](requirements-mcp.txt) — `mcp>=1.6,<4` |
| Glama / Docker run | [`Dockerfile`](Dockerfile), [`glama.json`](glama.json), [docs/GLAMA.md](docs/GLAMA.md) |
...
```json
{
  "mcpServers": {
    "aimarket-mcp-packager": {
      "command": "python",
      "args": ["-m", "aimarket_mcp_packager.stdio_server"]
    }
  }
}
```

Compat path (what Glama's admin form historically used — keep the shim file):
...
```json
{
  "mcpServers": {
    "aimarket-mcp-packager": {
      "command": "python",
      "args": ["plugins/aimarket-mcp-packager/mcp_stdio_server.py"]
    }
  }
}
```
...
Add the same stdio server under **Settings → MCP** (command + args pointing at `aimarket_mcp_packager/stdio_server.py` or the Docker image above).

---

## Python library (optional)

The same packaging logic is available as a library and as an AIMarket Hub plugin (PyPI package coming soon — for now `pip install -e .` from this directory, or use the AIMarket Hub Docker image). Hub HTTP routes are optional; **the MCP server runs standalone** via `aimarket_mcp_packager/stdio_server.py`.
