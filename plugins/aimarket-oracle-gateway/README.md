...
This repo ships `glama.json` + `Dockerfile`, so listing is one step: register/claim the server on
**glama.ai** pointing at this directory. Glama builds the Docker image and indexes the tools — after
which agents browsing Glama discover `get_random` / `compute_vdf` / … and call them directly.
