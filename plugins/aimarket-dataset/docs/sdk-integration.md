# aimarket-dataset — SDK Integration
...
assert any(p["name"] == "aimarket-dataset" for p in plugins["plugins"])

# 2. Example call
corpus = requests.get(f"{HUB}/ai-market/v2/p/aimarket-dataset/export/latest").json()
...
import aimarket_dataset  # adjust to package name
```
