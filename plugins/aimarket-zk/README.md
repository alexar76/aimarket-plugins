...
> Proving backend: the plugin loads and registers its routes with no backend configured, and the
> `/zk/*` endpoints then refuse with an explanation rather than the plugin vanishing from the
> catalogue. Set `AIMARKET_ZK_SIMULATED=1` for the development simulation — it is **not** real
> zero-knowledge and the hub says so at every call.
